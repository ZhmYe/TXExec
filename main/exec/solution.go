...
func newSolution(hashtable []map[string][]Unit) *Solution {
	solution := new(Solution)
	solution.peerNumber = len(hashtable)
	solution.result = make(map[string][]Unit, 0)
...
func (solution *Solution) combine(a map[string][]Unit, b map[string][]Unit) map[string][]Unit {
...
func solveConflictBaseLine(a []Unit, b []Unit) []Unit {
	if len(a) < len(b) {
		return b
	}
	return a
